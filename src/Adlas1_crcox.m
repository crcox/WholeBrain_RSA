...
% This file contains parts of code from SLOPE Toolbox version 1.0.
  ...
  while (true)
    ...
    % Lipschitz search
    [L,Aprods,X] = LipschitzSearch(proxFunction, A, B, Y, lambda, g, L, fPrev, Aprods,eta);
%     while (L < inf)
...
function [L,Aprods,X,r,f,q,d] = LipschitzSearch(proxFunction, A, B, Y, lambda, g, L, fPrev, Aprods,eta)
    while (L < inf)
      % Compute prox mapping
      X = proxFunction(Y - (1/L)*g, lambda/L);
      d = X - Y;
