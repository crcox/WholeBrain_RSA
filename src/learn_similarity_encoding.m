function [results,info] = learn_similarity_encoding(S, V, regularization, target_type, varargin)
  ...
  if VERBOSE
    fprintf('%5s%6s%11s %12s  %11s %11s  \n', 'cv','lam','lam1','test err','train err','nnz')
  end
  ...
  for i = 1:ncv
    ...
    for j = 1:nlam
      ...
      for k = 1:nlam1
        ...
        ix = find(nz_features);
        nvox = numel(nz_features);
        nzv = nnz(nz_features);

        % Prepare to evaluate solutions
        Wz = Uz(nz_rows,:) * Uz(nz_rows,:)'; % will include bias unit if present
        % N.B. V does not have bias unit appended; explicitly add bias to solution.
        Cz = bsxfun(@plus, V(:,nz_features) * Uz(nz_features, :), bias);
        Sz = Cz * Cz';
        St = C*C';
        ...
        if ~SMALL
          ix = find(nz_features); % does not include bias unit.
          [row,col] = ndgrid(ix, 1:r);
          if BIAS
        ...
        if VERBOSE
          fprintf('%3d | %6.2f | %6.2f | %10.2f | %10.2f | %10d\n', ...
            icv, lambda_j,lambda1_k,err1,err2,nnz(nz_rows));
        end
      end % lam1 loop
    end % lam loop
  end % cv loop
